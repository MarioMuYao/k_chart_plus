  ...
  void initRect(Size size) {
    ...
    for (int i = 0; i < secondaryStateLi.length; ++i) {
      mSecondaryRectList.add(RenderRect(
        Rect.fromLTRB(0, mMainRect.bottom + volHeight + i * secondaryHeight + mChildPadding, mWidth,
  ...
  getSecondaryMaxMinValue(int index, KLineEntity item) {
    ...
    switch (secondaryState) {
      ...
      case SecondaryState.MACD:
        if (item.macd != null) {
          mSecondaryRectList[index].mMaxValue =
              max(mSecondaryRectList[index].mMaxValue, max(item.macd!, max(item.dif!, item.dea!)));
          mSecondaryRectList[index].mMinValue =
              min(mSecondaryRectList[index].mMinValue, min(item.macd!, min(item.dif!, item.dea!)));
        }
        break;
      // KDJ
      case SecondaryState.KDJ:
        if (item.d != null) {
          mSecondaryRectList[index].mMaxValue =
              max(mSecondaryRectList[index].mMaxValue, max(item.k!, max(item.d!, item.j!)));
          mSecondaryRectList[index].mMinValue =
              min(mSecondaryRectList[index].mMinValue, min(item.k!, min(item.d!, item.j!)));
        }
        break;
      // RSI
